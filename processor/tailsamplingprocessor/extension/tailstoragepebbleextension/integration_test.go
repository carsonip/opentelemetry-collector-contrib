...
func TestTailSamplingWithPebbleTailStorageExtension(t *testing.T) {
	...
	extID := component.NewIDWithName(Type, "e2e")

	extCfg := extFactory.CreateDefaultConfig().(*Config)
	extCfg.Directory = t.TempDir()

	ext, err := extFactory.Create(t.Context(), extensiontest.NewNopSettings(Type), extCfg)
